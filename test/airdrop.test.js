...
describe("Deploying Contracts", function () {
  ...
  it("it should pause", async () => {
    await NuoToken.transfer(user1.address, parseEther("10")).should.be
      .fulfilled;
    await NuoToken.pause().should.be.fulfilled;
    await NuoToken.connect(user1)
      .transfer(user2.address, parseEther("10"))
      .should.be.rejectedWith("Pausable");
    await NuoToken.unpause().should.be.fulfilled;
    await NuoToken.connect(user1).transfer(user2.address, parseEther("10"))
      .should.be.fulfilled;
  });
  ...
  it("Should Deploy Staking Contract", async () => {
    ...
    StakingContract = await upgrades.deployProxy(
      Staking,
      [NuoToken.address, rest[0].address, NuoToken.address, 30],
      {
        initializer: "initialise",
      }
    );
    await StakingContract.deployed();
    STAKING_CONTRACT_ADDRESS = StakingContract.address;
  });
